...
enum Message {
    ...
    case userInput
    case inputError
    case startTask(number: Int, task: BankTask)
    case finishTask(number: Int, task: BankTask)
...
extension Message {
    ...
    func showMessage() -> String {
        ...
        case .startTask(let number, let task):
            return "\(number)번 고객 \(task.description)업무 시작"
        case .finishTask(let number, let task):
            return "\(number)번 고객 \(task.description)업무 종료"
